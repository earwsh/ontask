...
export default function AnalyticsTechnicalPage() {
  ...
  const deptBarData = data?.deptBreakdown?.map((d: any) => ({ status: d.deptName, count: d.total })) || [];

  return (
    <ProtectedRoute allowedRoles={['TECHNICAL_MANAGER', 'CEO', 'HR_MANAGER']}>
      <div className="h-full flex flex-col overflow-hidden animate-fade-in">
        <div className="shrink-0 mb-4">
          <h1 className="text-xl font-bold text-white mb-1">تحلیل جامع فنی</h1>
          <p className="text-text-muted text-sm">نمای کلی تسک‌ها، پروژه‌ها و دپارتمان‌ها</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 shrink-0 mb-4">
          ...
          <ProgressCard label="پروژه‌ها" value={data?.deptBreakdown?.length || 0} suffix="تیم" color="#06B6D4" />
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto space-y-4">
          ...
          {deptBarData.length > 0 && (
            <div className="bg-card border border-[rgba(255,255,255,0.06)] rounded-[20px] p-5">
              <h3 className="text-sm font-semibold text-white mb-3">تعداد تسک بر اساس دپارتمان</h3>
