    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        const [analyticsRes, tasksRes] = await Promise.all([
          api.get(`/analytics/department/${managed.id}`),
          api.get('/tasks'),
        ]);
        setStats({
          tasks: analyticsRes.data.total || 0,
          members: analyticsRes.data.memberPerformance?.length || 0,
          pendingApprovals: analyticsRes.data.pendingApprovals || 0,
          projects: analyticsRes.data.projectBreakdown?.length || 0,
        });
    ...
    <ProtectedRoute allowedRoles={['DEPARTMENT_MANAGER']}>
      <div className="animate-fade-in space-y-6">
        <div>
          ...
          <p className="text-text-muted text-sm mt-1">مدیریت وظایف و تیم</p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Link href="/dashboard/analytics/department"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[rgba(22,27,38,0.6)] p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <Link href="/dashboard/analytics/department" className="text-xs text-primary hover:text-primary-hover transition-colors">مشاهده همه</Link>
            </div>
            {loading ? (
                ...
                {pendingTasks.slice(0, 5).map((task: any) => (
                  <div key={task.id} className="flex items-center justify-between rounded-xl bg-[rgba(22,27,38,0.6)] px-4 py-3 transition-all duration-200 hover:bg-[rgba(30,37,52,0.6)]">
                    <div className="min-w-0 flex-1">
                      <Link href={`/dashboard/tasks/${task.id}`} className="text-sm text-white hover:text-primary transition-colors truncate block">{task.title}</Link>
